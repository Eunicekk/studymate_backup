...
<body>
	...
	<main class="postRegister-postWrapper">
		<form id="write-form" method="post"
			action="${pageContext.request.contextPath}/board/boardUpdateOk.bo?boardNumber=${board.getBoardNumber()}"
			...
			<section>
				...
				<div class="input-Title-wrap">
					<label for="input-labelText">제목</label> <input type="text"
						class="input-customInput" name="boardTitle"
						placeholder="글 제목을 입력해주세요! " value="${board.getBoardTitle()}">
				</div>
	...
	<script
		src="${pageContext.request.contextPath}/assets/js/boardWrite.js"></script>
...
<!-- 	<script>
		$('.writeButton-register').on(
				'click',
				function() {
					console.log($(this));
					window.location.href = '/board/boardUpdateOk.bo?boardNumber='
							+ $(this).data('boardnumber');
