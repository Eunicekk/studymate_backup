...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/assets/css/study_group_main.css" />
<link rel="stylesheet"
...
<body>
	...
	<main id="main">
		...
		<div class="searchContainer">
			<div class="searchBox">
				...
				<form id="search" action="" method="post">
					<input type="text" autocomplete="off" id="searchInput"
						name="searchInput" placeholder="찾으시는 키워드를 입력하세요" value="" /> <input
						type="submit" id="search-btn" value="" />
				</form>
				<a
					href="${pageContext.request.contextPath}/studyGroup/studyGroupWrite.sg?studyGroupNumber=${studyGroups.get(0).getStudyGroupNumber()}"
					class="newWriteBtn">새 글 쓰기</a>
			</div>
						...
						<div class="filterlist">
							<div class="InputContainer OnOff">
								<div class="selectText">
									<div class="TextFiled">
										<div class="Text on" [date-value="optValue" ]>진행 방식</div>
									</div>
									<div class="BtnFiled">
										<span></span>
										<div class="underImg">
					...
					<li class="filterBtn filed">
						<div class="filterlist">
							<div class="InputContainer">
								<div class="selectText">
									<div class="TextFiled">
										<div class="Text_filed" [date-value="optValue" ]>모집 분야</div>
									</div>
									<div class="BtnFiled">
										<span></span>
										<div class="underImg">
			...
			<!-- 스터디 카페 메인  -->
			<form action="/studyGroup/studyGroupMainOk.sg" method="get"
				name="order">
				<div class="mainSpace">
					<div class="titleOrder">
						<h2 class="title">스터디 그룹</h2>
						<input type="hidden" class="studygroup-count" value="${total }">
						<div class="selectOrder">
							<select id="order" name="order">
								<option id="latest" value="latest">최신순</option>
								<option id="viewCount" value="viewCount">조회순</option>
								<option id="interest" value="interest">좋아요순</option>
								<option id="commentCount" value="commentCount">댓글순</option>
							</select>
						</div>
					</div>
									...
									<li>
										...
										<section class="ReadReviewCnt">
											<div class="userInfo">
												<div class="userImg">
													<c:choose>
														<c:when test="${empty group.getProfileSystemName()}">
			...
			<ul class="pageNumber">
				...
				<c:if test="${prev}">
					<li class="prev">
						<button type="button"
							onclick="location.href='${pageContext.request.contextPath}/studyGroup/studyGroupMainOk.sg?page=${startPage - 1}'">
							<span class="box-sizing"> <!--  이 a 태그를 윤님은 button 에다가 onclick 을 주셨는데 나는 a 태그를 넣어봄. 되는지 확인하고 수정하기-->
								<img
				...
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:choose>
						<c:when test="${!(i==page)}">
							<li class="pageBtn">
								<button type="button"
									onclick="location.href='${pageContext.request.contextPath}/studyGroup/studyGroupMainOk.sg?page=${i}'">
									<c:out value="${i }" />
								</button>
							</li>
						</c:when>
				...
				<c:if test="${next }">
					<li class="next">
						<button type="button"
							onclick="location.href='${pageContext.request.contextPath}/studyGroup/studyGroupMainOk.sg?page=${endPage+1}'">
